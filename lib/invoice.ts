...
export async function listInvoiceByCustomer(customerId): Promise<any> {
  try {      
    if (!customerId) {
    ...
    const response = await fetch(`${apiEndpoint}?customerId=${customerId}&limit=10`, {
      headers: {
        "Authorization": `Bearer ${token}`,
        merchant: merchantId ,
      },
    ...
    if (!response.ok) {
      const text = await response.text()
      console.error("List invoice failed:", response.status, text)
      throw new Error(`List invoice failed: ${response.status}`)
    }
